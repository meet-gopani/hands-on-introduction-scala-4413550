object Challenges {
  ...
  /** You are working on a payment processing system. Implement processPayment,
    ...
    * The method outputs an `Either`.
    *
    * If the card balance is sufficient to cover the payment, return a Right
    * with the remaining balance after deduction.
    *
    * If the balance is insufficient, return a Left with an error message
